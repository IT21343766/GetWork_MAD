...
//  Click
    ...
    private fun showNameExisAlert(isNotvalid:Boolean){
        binding.etFullname.error= if (isNotvalid)"Name fill!" else null
    }

    private fun showTextMinimalAlert(isNotvalid: Boolean,text:String){
        if (text =="Username")
            binding.etUsername.error= if (isNotvalid) "$text morethan 6!" else null
        else if (text=="Password")
            binding.etPassword.error = if (isNotvalid) "$text more than 8!" else null
    }
